...
def _execute_engine(engine, strategy, user_message, user, student_context_getter, context_summary_getter, chatbot_settings):
    ...
    renvoie le texte produit, ou `None` si ce moteur échoue à répondre malgré
    la décision. `student_context_getter`/`context_summary_getter` : appels
    ...
    if engine == response_strategy.ENGINE_MATH:
        return math_engine.try_solve(user_message)

    if engine == response_strategy.ENGINE_RULE:
        return rule_engine.try_handle(user_message)
