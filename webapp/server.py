...
def api_data_export():
    ...
    de secret (pas de hash de mot de passe, pas de cookie/token). Volontairement
    non gatée par @requires_feature : c'est un export brut à but de portabilité
    des données (droit RGPD), pas le rapport PDF marketing (voir Feature.EXPORT
    sur /api/data/summary, qui alimente réellement js/pdfExport.js) — un accès
    à ses propres données ne doit jamais dépendre d'un palier d'abonnement."""
    user = request.current_user
    export = {
        "account": {
            "id": user["id"],
...
# ENABLE_DEV_DASHBOARD : coupe-circuit instantané (Mission 9) ; protégé par
# @login_required faute d'un système de rôle admin dans ce projet — à
# renforcer (rôle admin dédié) avant toute exposition publique réelle, voir
# rapport de phase.
