...
class TestCache(unittest.TestCase):
    ...
    def test_ttl_expire(self, mock_settings, mock_stats, mock_progress):
        ...
        # Clé (user_id, class_level) depuis l'audit multi-classe du chatbot —
        # jamais un cache partagé entre deux classes (voir TestClassLevel).
        cache_key = scr._cache_key(user["id"], None)
        ts, value = scr._cache[cache_key]
        scr._cache[cache_key] = (ts - scr._CACHE_TTL_SECONDS - 1, value)
        scr.resolve(user, use_cache=True)
...
class TestClassLevel(unittest.TestCase):
    """class_level est purement transporté par le resolver à cette étape :
    aucun appelant existant ne le fournit, donc "seconde" par défaut partout
    — c'est la garantie de non-régression de cette étape. Le resolver ne
    choisit jamais lui-même la classe (voir docstring de resolve())."""

    @patch("chatbot.student_context_resolver.read_user_course_progress")
    @patch("chatbot.student_context_resolver.read_user_stats")
    @patch("chatbot.student_context_resolver.read_user_settings")
    def test_defaut_seconde_si_non_fourni(self, mock_settings, mock_stats, mock_progress):
        mock_stats.return_value = _fake_stats()
        mock_settings.return_value = {"chatbot": {}, "favorites": [], "language": "fr"}
        mock_progress.return_value = {}
        scr.clear_cache()

        user = {"id": "class-level-test-1", "pseudo": "Gaël", "level": 1}
        context = scr.resolve(user, use_cache=False)

        self.assertEqual(context["class_level"], "seconde")

    @patch("chatbot.student_context_resolver.read_user_course_progress")
    @patch("chatbot.student_context_resolver.read_user_stats")
    @patch("chatbot.student_context_resolver.read_user_settings")
    def test_valeur_fournie_est_transportee_telle_quelle(self, mock_settings, mock_stats, mock_progress):
        mock_stats.return_value = _fake_stats()
        mock_settings.return_value = {"chatbot": {}, "favorites": [], "language": "fr"}
        mock_progress.return_value = {}
        scr.clear_cache()

        user = {"id": "class-level-test-2", "pseudo": "Hana", "level": 1}
        context = scr.resolve(user, class_level="premiere", use_cache=False)

        self.assertEqual(context["class_level"], "premiere")

    @patch("chatbot.student_context_resolver.read_user_course_progress")
    @patch("chatbot.student_context_resolver.read_user_stats")
    @patch("chatbot.student_context_resolver.read_user_settings")
    def test_appel_historique_sans_class_level_reste_valide(self, mock_settings, mock_stats, mock_progress):
        """Preuve de non-régression : un appel exactement comme avant cette
        étape (aucun paramètre class_level) produit un contexte valide."""
        mock_stats.return_value = _fake_stats(FAKE_HISTORY)
        mock_settings.return_value = _fake_settings()
        mock_progress.return_value = {}
        scr.clear_cache()

        user = {"id": "class-level-test-3", "pseudo": "Ivo", "level": 2}
        context = scr.resolve(user, use_cache=False)
...
class TestValidateur(unittest.TestCase):
    ...
    def test_class_level_absent_reste_valide(self):
        """class_level est optionnel : un StudentContext construit sans ce
        champ (comme tous ceux d'avant cette étape, ex. la fixture de
        test_response_strategy.py) ne doit produire AUCUNE erreur pour ça."""
        context = {
            "user_id": "x", "pseudo": "y", "niveau": "z", "niveau_brut": 1,
    ...
    def test_detecte_class_level_inconnu_du_registre(self):
        with patch("auth.read_user_course_progress", return_value={}), \
             patch("auth.read_user_stats", return_value=_fake_stats()), \
             patch("auth.read_user_settings", return_value={"chatbot": {}, "favorites": [], "language": "fr"}):
            scr.clear_cache()
            context = scr.resolve({"id": "validator-test-2", "pseudo": "Zoe", "level": 1}, use_cache=False)
        context["class_level"] = "terminale"  # pas encore déclaré dans curriculum_registry
        errors = validate_student_context(context)
        self.assertTrue(any("class_level" in e for e in errors))

    def test_accepte_class_level_premiere_deja_declare_au_registre(self):
        with patch("auth.read_user_course_progress", return_value={}), \
             patch("auth.read_user_stats", return_value=_fake_stats()), \
             patch("auth.read_user_settings", return_value={"chatbot": {}, "favorites": [], "language": "fr"}):
            scr.clear_cache()
            context = scr.resolve(
                {"id": "validator-test-3", "pseudo": "Wes", "level": 1}, class_level="premiere", use_cache=False,
            )
        self.assertEqual(validate_student_context(context), [])
