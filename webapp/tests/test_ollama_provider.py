...
class TestStreamChat(unittest.TestCase):
    ...
    def test_reponse_toujours_fermee_apres_stream_complet(self):
        """Durcissement production : `with resp:` doit fermer la connexion
        HTTP sortante dans le cas nominal, pas seulement sous exception."""
        lines = [json.dumps({"message": {"content": "ok"}, "done": True})]
        resp = _fake_response(lines=lines)
        with patch("chatbot.providers.ollama_provider.requests.post", return_value=resp):
            list(OllamaProvider().stream_chat(messages=[], system="s"))
        resp.__exit__.assert_called_once()

    def test_reponse_toujours_fermee_si_le_client_se_deconnecte_en_plein_stream(self):
        """Reproduit ce que fait server.py quand le navigateur ferme la
        connexion en plein streaming SSE : le générateur reçoit GeneratorExit
        au milieu de l'itération. Avant durcissement (`resp` sans `with`),
        rien ne garantissait la fermeture immédiate de la socket Ollama dans
        ce cas — voir audit production, fuite de connexion sous déconnexions
        fréquentes."""
        lines = [
            json.dumps({"message": {"content": "Bon"}}),
            json.dumps({"message": {"content": "jour"}}),
        ...
        resp = _fake_response(lines=lines)
        with patch("chatbot.providers.ollama_provider.requests.post", return_value=resp):
            gen = OllamaProvider().stream_chat(messages=[], system="s")
            next(gen)  # consomme le premier chunk, laisse le générateur en plein stream
            gen.close()  # équivalent à ce que fait Python quand le générateur est abandonné (GeneratorExit)
        resp.__exit__.assert_called_once()
