...
// ── Règle métier unique de progression (Phase N) ────────────────────────────
// Un chapitre est "Maîtrisé" seulement si l'accuracy dépasse ce seuil ET que
// l'élève a répondu à au moins ce nombre de questions dans ce chapitre —
// évite qu'un élève soit déclaré "Maîtrisé" après une ou deux bonnes
// réponses. Constantes centralisées ici : chapitres.js, dashboard.js et
// l'équivalent backend (webapp/chatbot/services/progress_service.py) doivent
// tous s'y référer, aucune page ne recalcule sa propre définition.
export const MASTERY_ACCURACY_THRESHOLD = 0.70;
export const MASTERY_MIN_ATTEMPTS = 10;

/** Statut de progression d'un chapitre : "not_started" | "in_progress" |
 * "mastered". Seule fonction habilitée à décider ce statut — chapitres.js et
 * dashboard.js doivent l'appeler plutôt que recalculer leur propre seuil. */
export function getChapterStatus(chapterId, history) {
  const m = masteryByChapter(history)[chapterId];
  if (!m || m.count === 0) return "not_started";
  if (m.rate >= MASTERY_ACCURACY_THRESHOLD && m.count >= MASTERY_MIN_ATTEMPTS) return "mastered";
  return "in_progress";
}
...
export function recordAnswer({ id, chapter, notion, difficulty, correct, usedHint, mode, duration_s }) {
  ...
  state.history.push({
    ...
    xp: gained,
    class_level: getStoredClassLevel(),
  });
...
// ── Isolation par classe (Seconde / Première...) ────────────────────────────
// L'historique et les séries sont stockés dans un seul objet par compte
// (voir persist()/load() ci-dessus), tagués `class_level` depuis recordAnswer/
// finalizeSeries. Les entrées enregistrées avant l'introduction du multi-classe
// n'ont pas ce champ : elles sont alors seconde par convention (déjà utilisée
// partout ailleurs dans le projet, voir curriculum_registry.py).
export function scopedStats(state, classLevel) {
  const belongsTo = (item) => (item.class_level || "seconde") === classLevel;
  const history = (state.history || []).filter(belongsTo);
...
export function saveInProgressSeries(snapshot) {
  localStorage.setItem(
    IN_PROGRESS_KEY,
    JSON.stringify({ ...snapshot, class_level: getStoredClassLevel(), savedAt: Date.now() }),
  );
}

/** Ne renvoie la série en pause que si elle appartient à la classe
 * actuellement active — chapter_id ("Chapitre_1"...) est partagé entre
 * classes, une série Première ne doit jamais apparaître "en cours" en
 * Seconde (et inversement). Absence de class_level (séries enregistrées
 * avant ce correctif) : seconde par convention, même repli que partout
 * ailleurs dans le projet. */
export function getInProgressSeries() {
  try {
    const snapshot = JSON.parse(localStorage.getItem(IN_PROGRESS_KEY));
    if (!snapshot) return null;
    return (snapshot.class_level || "seconde") === getStoredClassLevel() ? snapshot : null;
  } catch {
    return null;
  }
}
