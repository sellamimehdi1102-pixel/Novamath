...
function renderGeom(spec) {
  ...
  (spec.polygons || []).forEach((poly) => {
    ...
    content += `<polygon points="${pts}" class="geom-polygon${variantClass}${revealClass}"/>`;
  });
  ...
  (spec.texts || []).forEach((t) => {
    ...
    // `weight` (optionnel, ex. "big") et `anchor` (text-anchor SVG, pour
    // centrer un label à l'intérieur d'une forme) et `reveal` (révélation
    // progressive) — tous optionnels, ignorés si absents.
    ...
    content += `<text x="${x}" y="${y}" class="geom-label${weightClass}${revealClass}"${anchorAttr}>${t.label}</text>`;
  });
