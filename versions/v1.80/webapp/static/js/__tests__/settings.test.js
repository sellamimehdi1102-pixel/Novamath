...
const mockApi = {
  ...
  chatbotModels: vi.fn().mockResolvedValue({ models: [] }),
};
...
describe("settings.js — mountSettingsPanel", () => {
  it("rend le menu des 9 catégories", async () => {
    const container = await mountSettings();
    expect(container.querySelectorAll(".settings-menu-item")).toHaveLength(9);
  });
  ...
  it("charge le résumé de données et les sessions pour un compte non-invité", async () => {
    await mountSettings({ id: 1, is_guest: false });
    expect(mockApi.getDataSummary).toHaveBeenCalled();
    expect(mockApi.getSessions).toHaveBeenCalled();
  });

  it("ne charge ni données ni sessions pour un compte invité", async () => {
    await mountSettings({ id: 1, is_guest: true });
    expect(mockApi.getDataSummary).not.toHaveBeenCalled();
