...
export function getAccentColor() {
  return getComputedStyle(document.documentElement).getPropertyValue("--brand-indigo").trim() || "#6366f1";
}
export function getAccentColorSecondary() {
  return getComputedStyle(document.documentElement).getPropertyValue("--brand-violet").trim() || "#a855f7";
}
/** ex: accentRgba(0.28) -> "rgba(99, 102, 241, 0.28)", pour les canvas/SVG où
 * une variable CSS ne peut pas être injectée directement dans un attribut fill. */
export function accentRgba(alpha = 1) {
  const rgb = getComputedStyle(document.documentElement).getPropertyValue("--accent-rgb").trim() || "99, 102, 241";
...
export function initAppearance() {
  ...
  // le cache d'apparence (par défaut "dark" si l'utilisateur n'a encore
