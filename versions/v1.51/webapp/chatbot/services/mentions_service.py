...
# Catégories réelles du site — chacune a une vraie destination ou une vraie
# action, jamais une page qui n'existe pas.
CATEGORIES = [
    ...
    {"key": "series", "label": "Séries", "kind": "nav", "url": "chapitres.html"},
    {"key": "dashboard", "label": "Dashboard", "kind": "nav", "url": "dashboard.html"},
    {"key": "progression", "label": "Progression", "kind": "nav", "url": "dashboard.html"},
    {"key": "statistiques", "label": "Statistiques", "kind": "nav", "url": "dashboard.html"},
    {"key": "objectifs", "label": "Objectifs", "kind": "goals"},
    {"key": "quiz", "label": "Quiz", "kind": "nav", "url": "evaluation.html"},
    {"key": "profil", "label": "Profil", "kind": "nav", "url": "profil.html"},
    {"key": "parametres", "label": "Paramètres", "kind": "action", "action": "open-settings"},
...
def _dedupe_resources(results):
    ...
    best = {}
    for r in results:
        key = (r["type"], r.get("notion_id") or r.get("notion") or r["title"])
...
def build_grounding_block(mentions):
    """`mentions` : liste de dicts {type, chapter_id?, notion_id?, exercise_id?,
    label?} envoyés par le frontend (une par badge inséré dans le message).
    Résout chacune en vraie donnée — ignore silencieusement une mention
    invalide/périmée plutôt que de faire échouer toute la réponse."""
    if not mentions:
        return ""
    blocks = []
    for m in mentions:
