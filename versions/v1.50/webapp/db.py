...
def _deserialize_message(row):
    ...
    if msg.get("cards"):
        try:
            msg["cards"] = json.loads(msg["cards"])
        except json.JSONDecodeError:
            msg["cards"] = None
    return msg


def add_message(conversation_id, role, content, cards=None):
    conn = get_connection()
    try:
        cur = conn.execute(
            "INSERT INTO chatbot_messages (conversation_id, role, content, cards, created_at) VALUES (?, ?, ?, ?, ?)",
            (conversation_id, role, content, json.dumps(cards) if cards else None, _now()),
        )
        conn.commit()
        return cur.lastrowid
    finally:
