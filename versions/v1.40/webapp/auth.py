...
DEFAULT_SETTINGS = {
    ...
    "favorites": [],
    # Chatbot pédagogique (voir webapp/chatbot/) — Ollama (modèle local
    # Mistral) est le fournisseur par défaut, sans aucun appel à une API
    # cloud ; provider_manager.py permet d'en ajouter d'autres (Anthropic
    # déjà disponible en secours) sans migration de schéma.
    "chatbot": {
        "provider": "ollama",
        "model": "mistral",
        "temperature": 0.6,
