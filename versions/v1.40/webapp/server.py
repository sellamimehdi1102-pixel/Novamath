...
# ── Chatbot pédagogique (webapp/chatbot/) ────────────────────────────────────
# Résumé léger {id, title} des chapitres, calculé une seule fois depuis BANK
# déjà chargée ci-dessus — évite au context_builder de recharger
# exercises_bank.json pour un simple intitulé de chapitre.
...
def api_chatbot_health():
    """Testé au chargement de chatbot.html : si le fournisseur actif (Ollama
    par défaut) ne répond pas, le frontend affiche un bandeau clair au lieu
    de laisser l'utilisateur envoyer un message qui échouera."""
...
def api_chatbot_models():
    ...
    pour Ollama via /api/tags), alimente Paramètres → Chatbot."""
    settings = read_user_settings(request.current_user["id"])
    provider = (settings.get("chatbot") or {}).get("provider")
    return jsonify({
