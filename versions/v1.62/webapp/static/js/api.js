...
export const api = {
  ...
  chatbotGreeting: () => request("/api/chatbot/greeting"),
  ...
  chatbotMentions: (q, limit) => request(`/api/chatbot/mentions?q=${encodeURIComponent(q || "")}${limit ? `&limit=${limit}` : ""}`),

  // ── Recherche unifiée + objectifs quotidiens ────────────────────────────
  search: (q, scope, limit) => {
  ...
  chatbotStream: (conversationId, message, mentions, { signal } = {}) => {
    const csrfToken = readCookie("nm_csrf");
    return fetch(`/api/chatbot/conversations/${conversationId}/messages`, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...(csrfToken ? { "X-CSRF-Token": csrfToken } : {}) },
      credentials: "same-origin",
      body: JSON.stringify({ message, mentions: mentions && mentions.length ? mentions : undefined }),
      signal,
    });
  },
