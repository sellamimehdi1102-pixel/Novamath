...
_exercise_index_by_chapter = {}  # {chapter_id: retrieval_engine.Index} — construit à la demande


def _get_exercise_index_for_chapter(chapter_id):
    if chapter_id not in _exercise_index_by_chapter:
        docs = [d for d in _load_exercise_documents() if d["chapter_id"] == chapter_id]
        _exercise_index_by_chapter[chapter_id] = retrieval_engine.build_index(docs)
    return _exercise_index_by_chapter[chapter_id]


def search_exercises_in_chapter(chapter_id, query=None, difficulty=None, limit=5):
    ...
    if not chapter_id:
        return []
    chapter_docs = [d for d in _load_exercise_documents() if d["chapter_id"] == chapter_id]
    if difficulty:
    ...
    if query:
        index = _get_exercise_index_for_chapter(chapter_id)
    ...
    return [_exercice_result(d, score=1.0) for d in picked]


def exercises_by_topic(chapter_id, topic_id, limit=5):
    ...
    if not chapter_id or not topic_id:
        return []
    docs = [d for d in _load_exercise_documents() if d["chapter_id"] == chapter_id]
    matching = [d for d in docs if canonical_ids.resolve_topic_id(d["notion"], chapter_id=chapter_id) == topic_id]
    return [_exercice_result(d, score=1.0) for d in matching[:limit]]
...
def _exercice_result(m, score=None):
    return {
        ...
        "topic_id": canonical_ids.resolve_topic_id(m["notion"], chapter_id=m["chapter_id"]),
