...
def _weak_chapter_card(chapter_id):
    ...
    de cours indexée."""
    doc = next((d for d in knowledge_engine.all_documents() if d["chapter_id"] == chapter_id), None)
    if not doc:
        return None
    card = _course_card({
...
def build_cards(
    ...
    intent_chapter_id=None, weak_chapter_id=None,
):
    ...
    if weak_chapter_id:
        card = _weak_chapter_card(weak_chapter_id)
        if card:
            add(card)
    ...
    if weak_topics:
        ...
        topic = weak_topics[0]
        doc = knowledge_engine.get_notion(topic["chapter_id"], topic["topic_id"])
        if doc:
            card = _course_card({
                "chapter_id": doc["chapter_id"],
                "notion_id": doc["notion_id"],
            ...
            card["label_key"] = "chatbot_card_review_weak"
            add(card)
        exercise_matches = search_service.exercises_by_topic(topic["chapter_id"], topic["topic_id"], limit=5)
        if exercise_matches:
