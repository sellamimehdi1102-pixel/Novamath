...
_cache = {}  # user_id -> (timestamp, StudentContext dict)
...
def _cache_set(user_id, value):
    _cache[user_id] = (time.monotonic(), value)
...
def _normalize_mentions(mentions):
...
def _resolve_current_topic(current_chapter_ref, current_topic_ref, context_summary):
    chapter_id = canonical_ids.resolve_chapter_id(current_chapter_ref) if current_chapter_ref else None
    topic_id = None
    if current_topic_ref:
        topic_id = canonical_ids.resolve_topic_id(current_topic_ref, chapter_id=chapter_id)
...
def resolve(
    ...
    course_progress = read_user_course_progress(user_id)
    ...
    variables = variable_resolver.resolve(user, context_summary, effective_settings, stats=stats, settings=settings)
    chapter_id, topic_id = _resolve_current_topic(current_chapter_ref, current_topic_ref, context_summary)
    ...
    if use_cache:
        _cache_set(user_id, student_context)
