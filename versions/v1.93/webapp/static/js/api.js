...
// ── Toast centralisé "rate limited" (429, voir rate_limit_service.py) ───────
// Même élément/style que le toast quota ci-dessus (toast toast--error) mais
// jamais de redirection : une limite de fréquence se résout en patientant
// `retry_after` secondes, pas en changeant de page. Un seul timer partagé
// (rateLimitToastTimer) évite d'empiler deux messages si plusieurs requêtes
// rate-limitées reviennent coup sur coup.
let rateLimitToastTimer = null;

export function handleRateLimited(payload) {
  const seconds = Math.max(1, Math.ceil(payload.retry_after || 1));
  let el = document.getElementById("novamath-quota-toast");
  if (!el) {
    el = document.createElement("div");
    el.id = "novamath-quota-toast";
  ...
  el.className = "toast toast--error";
  el.innerHTML = `<span>Trop de tentatives. Réessaie dans ${seconds} seconde${seconds > 1 ? "s" : ""}.</span>`;
  el.hidden = false;
  rateLimitToastTimer = setTimeout(() => {
    el.hidden = true;
  }, Math.min(seconds * 1000, 8000));
}
...
function buildApiError(path, status, payload) {
  ...
  // ci-dessus (volume quotidien) — même traitement centralisé : toast ici,
  // l'appelant n'a qu'à arrêter proprement son propre état de chargement.
...
export const api = {
  ...
  enable2FA: (code) => request("/api/auth/2fa/enable", { method: "POST", body: { code } }),
  disable2FA: (password, code) => request("/api/auth/2fa/disable", { method: "POST", body: { password, code } }),
  // Anonymes (défi de connexion en cours, voir login()) : jamais de jeton CSRF
  // à envoyer, aucune session n'existe encore à ce stade.
  verify2FA: (challengeToken, code) =>
    request("/api/auth/2fa/verify", { method: "POST", body: { challenge_token: challengeToken, code } }),
  recover2FA: (challengeToken, recoveryCode) =>
    request("/api/auth/2fa/recovery", { method: "POST", body: { challenge_token: challengeToken, recovery_code: recoveryCode } }),

  // ── Consentement parental RGPD (voir consent_service.py) — routes publiques,
  // ouvertes depuis le lien reçu par email par le parent, sans session. ─────
  getParentalConsentStatus: (token) => request(`/api/auth/parental-consent/${encodeURIComponent(token)}`),
  acceptParentalConsent: (token) => request(`/api/auth/parental-consent/${encodeURIComponent(token)}/accept`, { method: "POST" }),
  refuseParentalConsent: (token) => request(`/api/auth/parental-consent/${encodeURIComponent(token)}/refuse`, { method: "POST" }),
  resendParentalConsent: (email, password) =>
  ...
  setCookieConsent: (statistics, marketing) =>
    request("/api/privacy/cookie-consent", { method: "PUT", body: { statistics, marketing } }),
  ...
  billingCustomerPortal: () => request("/api/billing/customer-portal", { method: "POST" }),
  billingChangePlan: (plan) => request("/api/billing/change-plan", { method: "POST", body: { plan } }),
