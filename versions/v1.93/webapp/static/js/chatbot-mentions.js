...
export function initMentions({ input, onNavigate, onAction }) {
  ...
  async function runQuery(query) {
    renderLoading();
    try {
      const { items, did_you_mean } = await api.chatbotMentions(query, 8, getStoredClassLevel());
  ...
  function selectItem(item) {
    ...
    if (item.item_kind === "category" && item.kind === "data") {
      // Progression/Statistiques/Dashboard/Quiz : récupère les vraies données
      // de l'élève et les insère comme un badge (comme cours/exercice), au
      // lieu de rediriger vers une autre page — le backend résoudra ce badge
      // en chiffres réels au moment de l'envoi (mentions_service.py).
      const chip = composer.createMentionChip({ type: "data", data_key: item.data_key, label: item.label });
