...
async function toggleReview(row, series) {
  ...
  cell.innerHTML = "";
  details.forEach(({ q, ex }) => {
    const item = document.createElement("div");
    item.className = "series-review-item";

    const enonce = document.createElement("span");
    if (ex) {
      setMathContent(enonce, ex.enonce);
    } else {
      enonce.textContent = "(exercice indisponible)";
    }

    const badge = document.createElement("span");
    badge.className = `badge ${q.correct ? "badge--success" : "badge--danger"}`;
    badge.textContent = q.correct ? "Réussi" : "Échoué";

    item.append(enonce, badge);
