...
def resolve(user, context_summary, chatbot_settings=None, stats=None, settings=None, class_level=None):
    ...
    `class_level` (optionnel, "seconde" par défaut) : filtre l'historique par
    classe avant de calculer meilleur/pire chapitre — même règle que
    context_builder.build_context_summary (voir scope_by_class_level), pour
    ne jamais mélanger les chapitres de deux classes différentes."""
    ...
    history = context_builder.scope_by_class_level(stats.get("history", []), class_level)
    ...
    return {
        ...
        "nombre_quiz": len(context_builder.scope_by_class_level(stats.get("series") or [], class_level)),
