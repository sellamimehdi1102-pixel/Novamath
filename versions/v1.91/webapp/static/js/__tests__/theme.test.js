...
describe("theme.js — applyAppearance", () => {
  ...
  it("ignore un accent/fontSize invalide (retombe sur défaut)", () => {
    applyAppearance({ accent: "invalide", fontSize: "gigantesque" });
    const root = document.documentElement;
    expect(root.getAttribute("data-accent")).toBe("purple");
    expect(root.getAttribute("data-font-size")).toBe("normal");
  });

  it("accepte un accent/fontSize valide", () => {
    applyAppearance({ accent: "blue", fontSize: "large" });
    const root = document.documentElement;
