...
def _try_exercise_answer(intent_result, chatbot_settings, user_message, class_level=None):
    ...
    resolved = search_service.resolve(
        search_service.SCOPE_EXERCICES, exercise_id=matched[0]["exercise_id"], class_level=class_level,
    )
    if not resolved:
        return NO_EXERCISE_MESSAGE
    text = (
...
def try_answer(intent_result, user, context_summary, chatbot_settings, user_message=None, student_context=None):
    ...
    `student_context` (optionnel, Student Context Resolver v2) : permet à
    response_composer.compose() d'éviter de répéter une formulation déjà
    utilisée dans la conversation (phrasing_memory.py) — comportement
    inchangé si omis (choix purement aléatoire, comme avant)."""
    intent = intent_result["intent"]

    if intent in intent_service.LOCAL_DATA_INTENTS:
        variables = variable_resolver.resolve(
            user, context_summary, chatbot_settings, class_level=(student_context or {}).get("class_level"),
        )
        text = response_composer.compose(intent, variables, chatbot_settings, student_context=student_context)
        if text is not None:
            return text

    if intent == intent_service.EXERCICE and intent_result["chapter_id"]:
        return _try_exercise_answer(
            intent_result, chatbot_settings, user_message,
