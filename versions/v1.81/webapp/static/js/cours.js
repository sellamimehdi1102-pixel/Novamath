...
function openNotionReader(chapterId, content, notion) {
  ...
  // ── Corps de la leçon (définitions, méthode, exemples...) — extrait dans
  // sa propre variable pour pouvoir soit l'afficher pleine largeur, soit le
  // glisser dans la colonne "explications" à côté du graphique quand la
  // notion en a un (voir cours-figure-layout ci-dessous). ────────────────
  const bodyHtml = `
  ...
  // ── Figure : quand la notion en a une, on bascule en disposition deux
  // colonnes (graphique ~45% à gauche, explications ~55% à droite, colonne
  // graphique "collante" pour rester visible pendant la lecture) ; sinon le
  // corps reste en pleine largeur. Sur mobile, le graphique passe avant le
  // texte (voir cours.css, breakpoint deux colonnes). ─────────────────────
  const figureHtml = notion.figure ? `
    <div class="cours-figure-wrap">
      ${renderFigure(notion.figure)}
      ${notion.figure.alt ? `<div class="cours-figure-caption">${notion.figure.alt}</div>` : ""}
    </div>
  ` : "";
  ...
  readerView.innerHTML = `
    ...
    <div class="cours-figure-layout">
      <div class="cours-figure-col">${figureHtml}</div>
      <div class="cours-figure-text-col">${bodyHtml}</div>
    </div>
    ` : bodyHtml}
