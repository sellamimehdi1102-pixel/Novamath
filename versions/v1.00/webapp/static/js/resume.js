...
export function renderResumeCard(container) {
  ...
  const progressPct = Math.round((s.seriesIndex / SERIES_TOTAL) * 100);

  container.hidden = false;
  container.innerHTML = `
    <div style="display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;">
      <div>
        <h3 style="font-size:1rem; margin-bottom:10px;">📖 Série en cours</h3>
        <div style="display:flex; gap:18px; flex-wrap:wrap; font-size:0.85rem; color:var(--text-muted); margin-bottom:8px;">
          ...
          <span>Question <strong style="color:var(--text);">${s.seriesIndex + 1} / ${SERIES_TOTAL}</strong></span>
