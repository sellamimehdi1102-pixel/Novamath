...
function openNotionReader(chapterId, content, notion) {
  ...
  // gauche, son explication dédiée à droite), insérée dans le fil de lecture
  // pleine largeur exactement là où la figure apparaît naturellement (juste
  // après la Définition) — le reste du cours reste en une seule colonne. La
  // colonne de droite affiche `figure.details` (liste de points qui font
  // directement référence au graphique, ex. "Le point A est ici...") si
  // fourni, sinon la légende `figure.alt` en repli. Pas de position sticky :
  // si l'explication est plus longue que le graphique, elle continue
  // naturellement dessous sans espace vide (la grille aligne juste le haut
  // des deux colonnes). ────────────────────────────────────────────────────
  function buildFigureBlockHtml(figure) {
    if (!figure) return "";
    const hasDetails = figure.details?.length;
    const rightCol = hasDetails ? `
        <div class="cours-figure-details-title">${icon("compass")} Sur ce graphique</div>
        <ul class="cours-figure-details-list">
          ${figure.details.map((d) => `<li data-text="${encodeURIComponent(d)}"></li>`).join("")}
        </ul>
