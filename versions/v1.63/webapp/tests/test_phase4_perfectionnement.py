...
correction sur les données réelles du projet, et (Mission 9) que le hook
d'observabilité ajouté à `conversation_manager.py` reste désactivable sans
changer la réponse.
"""
import unittest
...
class TestLLMCallAnalytics(unittest.TestCase):
    ...
    def test_record_et_snapshot(self):
        strategy = rs.decide_strategy("Question totalement hors programme sur les licornes", REAL_USER, use_cache=False)
...
class TestMission9NonRegression(unittest.TestCase):
    ...
    def test_analytics_desactive_ne_journalise_rien(self):
        ...
        conv = cm.create_conversation(REAL_USER["id"], "Test Mission 9")
        try:
            list(cm.stream_reply(REAL_USER, conv["id"], "Raconte-moi une blague sur les licornes qui font du velo"))
        finally:
    ...
    def test_analytics_active_ne_change_pas_la_reponse(self):
        ...
        cm.ENABLE_LLM_CALL_ANALYTICS = False
        conv = cm.create_conversation(REAL_USER["id"], "Test Mission 9 A")
        try:
            reponse_sans = "".join(cm.stream_reply(REAL_USER, conv["id"], message))
        finally:
            cm.delete_conversation(conv["id"], REAL_USER["id"])

        cm.ENABLE_LLM_CALL_ANALYTICS = True
        conv = cm.create_conversation(REAL_USER["id"], "Test Mission 9 B")
        try:
            reponse_avec = "".join(cm.stream_reply(REAL_USER, conv["id"], message))
        finally:
            cm.delete_conversation(conv["id"], REAL_USER["id"])
    ...
    def test_panne_de_lanalytics_nempeche_pas_la_reponse(self):
        ...
        with patch(
            ...
            conv = cm.create_conversation(REAL_USER["id"], "Test Mission 9 panne")
            try:
                texte = "".join(cm.stream_reply(REAL_USER, conv["id"], "Raconte-moi une blague sur les licornes"))
            finally:
                cm.delete_conversation(conv["id"], REAL_USER["id"])
