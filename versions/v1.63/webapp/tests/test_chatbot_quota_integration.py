...
class TestDepassementDeQuota429(ChatbotQuotaTestCase):
    def test_free_bloque_au_26e_message_avec_le_format_exact(self):
        _seed_usage(self.user["id"], 25)  # amène directement à la limite Free
        resp = _send_message(self.client, self.headers, self.conv_id)
    ...
    def test_25e_message_reussit_le_26e_echoue(self):
        _seed_usage(self.user["id"], 24)
        ok = _send_message(self.client, self.headers, self.conv_id, "Le 25e")
        self.assertEqual(ok.status_code, 200)
        blocked = _send_message(self.client, self.headers, self.conv_id, "Le 26e")
        self.assertEqual(blocked.status_code, 429)

    def test_premium_bloque_a_250_avec_required_plan_ultra(self):
        _set_plan(self.user["id"], Plan.PREMIUM)
        _seed_usage(self.user["id"], 250)
        resp = _send_message(self.client, self.headers, self.conv_id)
        self.assertEqual(resp.status_code, 429)
        body = resp.get_json()
    ...
    def test_ultra_nest_jamais_bloque(self):
        _set_plan(self.user["id"], Plan.ULTRA)
        _seed_usage(self.user["id"], 10_000)
        resp = _send_message(self.client, self.headers, self.conv_id)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_apres_429_le_compteur_ne_depasse_pas_la_limite(self):
        ...
        self.assertEqual(_used(self.client), 25)
...
class TestRegenerateEtRetry(ChatbotQuotaTestCase):
    ...
    def test_regenerate_bloque_si_quota_epuise(self):
        ...
        _seed_usage(self.user["id"], 24)  # usage total = 25, à la limite
        resp = self.client.post(
            f"/api/chatbot/conversations/{self.conv_id}/regenerate",
            json={}, headers=self.headers,
        )
...
class TestApiQuotaRoute(ChatbotQuotaTestCase):
    ...
    def test_reflete_lusage_apres_consommation(self):
        ...
        body = self.client.get("/api/quota").get_json()
        self.assertEqual(body["chat_messages"], {
            "used": 1, "remaining": 24, "limit": 25, "unlimited": False,
        })
    ...
    def test_premium_chat_messages(self):
        ...
        body = self.client.get("/api/quota").get_json()
        self.assertEqual(body["chat_messages"], {
            "used": 0, "remaining": 250, "limit": 250, "unlimited": False,
        })

    def test_ultra_chat_messages_illimite(self):
        _set_plan(self.user["id"], Plan.ULTRA)
        body = self.client.get("/api/quota").get_json()
        self.assertEqual(body["chat_messages"], {
            "used": 0, "remaining": None, "limit": None, "unlimited": True,
        })

    def test_ultra_reste_illimite_apres_grosse_consommation(self):
        _set_plan(self.user["id"], Plan.ULTRA)
        _seed_usage(self.user["id"], 1_000)
        body = self.client.get("/api/quota").get_json()
        self.assertEqual(body["chat_messages"]["unlimited"], True)
        self.assertIsNone(body["chat_messages"]["limit"])
        self.assertIsNone(body["chat_messages"]["remaining"])
...
class TestFormatLegacyChatbotQuota(ChatbotQuotaTestCase):
    ...
    def test_forme_historique_used_limit(self):
        ...
        self.assertEqual(body, {"used": 0, "limit": 25})
    ...
    def test_limit_none_pour_ultra(self):
        _set_plan(self.user["id"], Plan.ULTRA)
        body = self.client.get("/api/chatbot/quota").get_json()
...
class TestReinitialisationQuotidienne(ChatbotQuotaTestCase):
    def test_usage_dun_autre_jour_najoute_rien_aujourdhui(self):
        _seed_usage(self.user["id"], 25, day="2020-01-01")
...
class TestConversationManagerDelegation(unittest.TestCase):
    ...
    def test_quota_status_format_historique(self):
        ...
        self.assertEqual(status, {"used": 1, "limit": 25})
