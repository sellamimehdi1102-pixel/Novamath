...
def _migrate_guest_lifecycle_columns(conn):
    ...
    for ddl in (
        ...
        "ALTER TABLE chatbot_messages ADD COLUMN cards TEXT",
        # Mentions "@" résolues (système de mentions revu) : JSON nullable.
        "ALTER TABLE chatbot_messages ADD COLUMN mentions TEXT",
...
def mark_stripe_event_processed(event_id, event_type):
    """INSERT OR IGNORE : si deux requêtes concurrentes traitent le même event_id
    (double livraison quasi simultanée), la seconde insertion est ignorée sans
    erreur au lieu de violer la contrainte PRIMARY KEY."""
    conn = get_connection()
    try:
        conn.execute(
