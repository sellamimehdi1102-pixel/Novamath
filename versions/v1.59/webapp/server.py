...
from chatbot.conversation_manager import QuotaExceeded
...
def api_chatbot_messages(conversation_id):
    ...
    def generate():
        try:
            for chunk in conversation_manager.stream_reply(user, conversation_id, text, _CHATBOT_CHAPTERS_SUMMARY, mentions=mentions):
                yield f"data: {json.dumps({'delta': chunk})}\n\n"
            cards = conversation_manager.attach_action_cards(user, conversation_id, _CHATBOT_CHAPTERS_SUMMARY)
            yield f"data: {json.dumps({'done': True, 'cards': cards})}\n\n"
        except QuotaExceeded as e:
            yield f"data: {json.dumps({'error': str(e), 'quota_exceeded': True})}\n\n"
...
def api_chatbot_regenerate(conversation_id):
    ...
    def generate():
        try:
            for chunk in conversation_manager.regenerate_last(user, conversation_id, _CHATBOT_CHAPTERS_SUMMARY):
                yield f"data: {json.dumps({'delta': chunk})}\n\n"
            cards = conversation_manager.attach_action_cards(user, conversation_id, _CHATBOT_CHAPTERS_SUMMARY)
            yield f"data: {json.dumps({'done': True, 'cards': cards})}\n\n"
        except QuotaExceeded as e:
...
def api_chatbot_quota():
    return jsonify(conversation_manager.quota_status(request.current_user["id"]))
...
def api_chatbot_context_preview():
    ...
    summary = build_context_summary(request.current_user, _CHATBOT_CHAPTERS_SUMMARY)
...
def api_chatbot_health():
    """Testé au chargement de chatbot.html : si le fournisseur actif (Anthropic
    par défaut) ne répond pas — clé API absente/invalide, réseau coupé... —
    le frontend affiche un bandeau clair au lieu de laisser l'utilisateur
    envoyer un message qui échouera."""
    override = request.args.get("provider")
    if override:
        provider = override
    else:
        settings = read_user_settings(request.current_user["id"])
        provider = (settings.get("chatbot") or {}).get("provider")
    return jsonify(provider_manager.health_check(provider))
...
def api_chatbot_mentions():
    ...
    return jsonify(search_mentions(query, limit=limit))


@app.route("/api/chatbot/models")
@login_required
def api_chatbot_models():
    """Modèles disponibles pour le fournisseur actif (détectés dynamiquement
    pour Ollama via /api/tags ; catalogue statique pour Anthropic), alimente
    Paramètres → Chatbot."""
    settings = read_user_settings(request.current_user["id"])
    provider = (settings.get("chatbot") or {}).get("provider")
    return jsonify({
        "provider": provider_manager.active_provider_name(provider),
        "models": provider_manager.available_models(provider),
    })
...
def api_search():
    ...
    return jsonify({"results": search_service.search(query, scope=scope, limit=limit)})
