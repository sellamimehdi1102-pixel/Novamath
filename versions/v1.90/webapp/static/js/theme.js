...
/** Certaines pages (ex. index.html) chargent plusieurs scripts qui appellent
 * chacun bindThemeToggle sur le même bouton (landing.js + initSettingsManager
 * via abonnement.js) — sans garde, ça pose deux écouteurs "click" distincts :
 * un seul clic déclenche alors toggleTheme() deux fois de suite et annule le
 * changement (dark puis retour immédiat à light). Idempotent par élément. */
export function bindThemeToggle(buttonEl) {
  if (!buttonEl || buttonEl.dataset.themeToggleBound) return;
  buttonEl.dataset.themeToggleBound = "true";
...
const FONT_SIZE_VALUES = new Set(["normal", "large"]);
