...
# class_level n'est volontairement PAS dans REQUIRED_TOP_LEVEL_SCALAR_FIELDS :
# il doit rester optionnel (voir student_context_resolver.resolve()) pour que
# tout StudentContext construit avant cette étape — y compris dans des tests
# déjà existants ailleurs dans le projet — reste valide sans modification.
# Sa valeur, quand elle est présente, est quand même vérifiée plus bas.
...
def validate_student_context(context):
    ...
    # class_level : jamais une valeur inventée — doit correspondre à un
    # programme réellement déclaré dans curriculum_registry.py (registre
    # purement déclaratif, sans lien avec le comportement actuel du resolver).
    ...
    if class_level is not None and class_level not in curriculum_registry.CURRICULUM_REGISTRY:
        errors.append(f"class_level={class_level!r} n'est pas un programme connu du registre")
