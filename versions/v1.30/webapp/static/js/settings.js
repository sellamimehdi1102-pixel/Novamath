...
const CHATBOT_PROVIDERS = [
  ...
  { value: "mistral", label: "Mistral (bientôt disponible)", disabled: true },
];
...
function renderChatbot() {
  ...
  return `
    ...
    <div class="settings-section">
      ...
      <p class="settings-panel-desc" style="margin-bottom:10px;">Architecture prête pour en accueillir d'autres (OpenAI, Gemini, Mistral…) — un seul est branché pour l'instant.</p>
      <div class="choice-group" data-group="chatbotProvider">
        ${CHATBOT_PROVIDERS.map((p) => `<button type="button" class="choice-pill${p.value === c.provider ? " active" : ""}" data-value="${p.value}" ${p.disabled ? "disabled" : ""}>${p.label}</button>`).join("")}
      </div>
      <div class="settings-section-title" style="margin-top:18px;">Modèle</div>
      <div class="choice-group" data-group="chatbotModel">
        ${CHATBOT_MODELS.map((m) => `<button type="button" class="choice-pill${m.value === c.model ? " active" : ""}" data-value="${m.value}">${m.label}</button>`).join("")}
      </div>
    </div>
...
function bindChatbot() {
  bindPillGroup(panel, "chatbotProvider", (value) => updateSetting("chatbot", "provider", value));
