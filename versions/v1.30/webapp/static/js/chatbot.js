...
// ── Pièces jointes (Phase 1 : placeholder, traitement réel en Phase 2) ──────
function attachPlaceholder(label) {
  input.value = `${input.value ? input.value + " " : ""}[${label} — pièce jointe non traitée pour l'instant]`;
  autoResize();
  sendBtn.disabled = !input.value.trim();
  input.focus();
}
$("chatbot-attach-image").addEventListener("click", () => attachPlaceholder("Image"));
$("chatbot-attach-pdf").addEventListener("click", () => attachPlaceholder("PDF"));
$("chatbot-attach-exercise").addEventListener("click", () => attachPlaceholder("Exercice"));
