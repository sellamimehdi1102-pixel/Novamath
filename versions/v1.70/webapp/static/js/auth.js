...
$("signup-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (result.account_status === "pending_parental_consent") {
      ...
      if (result.dev_consent_link) {
        // Aucun service d'envoi d'email configuré en local (voir .env.example
        // EMAIL_SMTP_HOST) : lien affiché uniquement en console pour tester le
        // flux sans vrai email — jamais présent en production.
        console.info("[dev] Lien de consentement parental :", result.dev_consent_link);
      }
      return;
    }
    redirectAfterAuth();
