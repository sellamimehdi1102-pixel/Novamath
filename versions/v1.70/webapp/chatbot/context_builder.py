...
def scope_by_class_level(entries, class_level):
    """Filtre une liste d'entrées d'historique/séries (chacune taguée
    `class_level` par store.js::recordAnswer/saveInProgressSeries) sur la
    classe active — même convention que store.js::scopedStats côté frontend :
    une entrée sans `class_level` (historique antérieur à l'introduction du
    multi-classe) est traitée comme "seconde", jamais perdue ni mélangée à
    une autre classe. Réutilisé par variable_resolver.py pour ne jamais
    dupliquer cette règle."""
    target = class_level or "seconde"
    return [e for e in (entries or []) if (e.get("class_level") or "seconde") == target]
...
def build_context_summary(user, chapters_summary=None, stats=None, settings=None, course_progress=None, class_level=None):
    ...
    `class_level` (optionnel, "seconde" par défaut) : l'historique d'exercices
    (auth.read_user_stats) n'est PAS séparé par classe au niveau du stockage —
    chaque entrée porte son propre `class_level` (store.js::recordAnswer),
    exactement comme côté frontend (store.js::scopedStats). Sans ce filtre,
    les notions faibles/maîtrisées d'un élève ayant déjà travaillé en Seconde
    ET en Première seraient mélangées ; voir scope_by_class_level()."""
    ...
    course_progress = course_progress if course_progress is not None else read_user_course_progress(user["id"], class_level=class_level)
    history = scope_by_class_level(stats.get("history", []), class_level)
