...
def search_mentions(query, limit=MAX_ITEMS, class_level=None):
    ...
    if query and remaining:
        ...
        cours = search_service.search(query, scope=[search_service.SCOPE_COURS], limit=remaining, class_level=class_level)
        exercices_raw = search_service.search(query, scope=[search_service.SCOPE_EXERCICES], limit=remaining * 6, class_level=class_level)
    ...
    guess = search_service.best_guess(query, class_level=class_level)
    if guess:
...
def build_grounding_block(mentions, user=None, chapters_summary=None, class_level=None):
    ...
    for m in mentions:
        if m.get("type") == "data":
            if not user:
                continue
            summary = context_builder.build_context_summary(user, chapters_summary, class_level=class_level)
