...
def _weak_chapter_card(chapter_id, class_level=None):
    """Carte "Revoir ce chapitre" pour le chapitre le plus faible de l'élève
    (variable_resolver.py::chapitre_le_plus_faible, Phase S) — cherche une
    notion réelle de ce chapitre via knowledge_engine (même source que le
    RAG), jamais une donnée inventée. None si le chapitre n'a pas de fiche
    de cours indexée."""
    doc = next((d for d in knowledge_engine.all_documents(class_level=class_level) if d["chapter_id"] == chapter_id), None)
    if not doc:
        return None
    card = _course_card({
        ...
        "title": doc["title"],
        "url": f"cours.html?chapter={doc['chapter_id']}&notion={doc['notion_id']}",
    })
...
def build_cards(
    user_message, assistant_response, context_summary, search_results=None,
    intent_chapter_id=None, weak_chapter_id=None, class_level=None,
):
    ...
    `weak_chapter_id` : chapitre le plus faible de l'élève (Phase S,
    variable_resolver.py) quand la réponse portait sur sa progression/ses
    statistiques — propose de le revoir directement."""
    cards = []
    seen = set()
    ...
    if weak_chapter_id:
        card = _weak_chapter_card(weak_chapter_id, class_level=class_level)
        if card:
            add(card)
    ...
    weak_topics = context_summary.get("weak_topics") or []
    if weak_topics:
        # Résolution exacte (canonical_ids.py) : topic_id déjà connu, plus
        # besoin de repasser par une recherche floue texte libre.
        topic = weak_topics[0]
        doc = knowledge_engine.get_notion(topic["chapter_id"], topic["topic_id"], class_level=class_level)
        if doc:
            card = _course_card({
                "chapter_id": doc["chapter_id"],
                "notion_id": doc["notion_id"],
            ...
            card["label_key"] = "chatbot_card_review_weak"
            add(card)
        exercise_matches = search_service.exercises_by_topic(
            topic["chapter_id"], topic["topic_id"], limit=5, class_level=class_level,
        )
        if exercise_matches:
        ...
        # Repli (texte libre non résolu en topic_id, ex. notion pas encore
        # couverte par static/data/topic_crosswalk.json) : ancien chemin par
        # recherche floue, inchangé.
        for weak_notion in (context_summary.get("weak_notions") or [])[:1]:
            matches = search_service.search(weak_notion, scope=["cours"], limit=1, class_level=class_level)
            if matches and matches[0]["score"] >= SCORE_THRESHOLD:
                card = _course_card(matches[0])
                card["label_key"] = "chatbot_card_review_weak"
                add(card)
            exercise_matches = search_service.search(weak_notion, scope=["exercices"], limit=5, class_level=class_level)
            if exercise_matches:
                add(_notion_series_card(
